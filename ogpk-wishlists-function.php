...
if ( ! function_exists( 'ogpk_get_option' ) ) {
	 ...
	 * @return mixed
	 */
	function ogpk_get_option( $category, $option = '' ) {
		$prefix = OGPKWL_PREFIX . '-';
		$values = get_option( $prefix . $category, array() );
		if ( empty( $values ) ) {
			$values = ogpk_get_option_defaults( $category );
		}
		if ( empty( $option ) ) {
			return $values;
		} else {
...
if ( ! function_exists( 'ogpk_get_option_admin' ) ) {
	 ...
	 * @return mixed
	 */
	function ogpk_get_option_admin( $category, $option = '' ) {
		$prefix = OGPKWL_PREFIX . '-';
		$values = get_option( $prefix . $category, array() );
		if ( empty( $values ) ) {
			$values = array();
...
if ( ! function_exists( 'ogpk_style' ) ) {
	 ...
	 * @return string
	 */
	function ogpk_style( $selector = '', $element = '' ) {
		$key    = md5( $selector . '||' . $element );
		$values = get_option( OGPKWL_PREFIX . '-style_options', array() );
		if ( empty( $values ) ) {
			return '';
		}
...
if ( ! function_exists( 'ogpk_update_option' ) ) {
	 ...
	 * @return boolean
	 */
	function ogpk_update_option( $category, $option = '', $value = false ) {
		$prefix = OGPKWL_PREFIX . '-';
		if ( empty( $option ) ) {
			if ( is_array( $value ) ) {
...
class OGpkWLRename {
	 ...
	 */
	function rename() {
		$this->rename        = ogpk_get_option( 'rename', 'rename' );
		$this->rename_single = ogpk_get_option( 'rename', 'rename_single' );
		$this->rename_plural = ogpk_get_option( 'rename', 'rename_plural' );
...
$ogpkwl_rename = new OGpkWLRename();
$ogpkwl_rename->rename();

if ( ! function_exists( 'ogpk_wishlist_template' ) ) {
	 ...
	 */
	function ogpk_wishlist_template( $template_name, $args = array(), $template_path = '' ) {
...
if ( ! function_exists( 'ogpk_wishlist_locate_template' ) ) {
	 ...
	 * @return mixed
	 */
	function ogpk_wishlist_locate_template( $template_name, $template_path = '', $default_path = '' ) {
		$prefix = 'ti-';
		...
		if ( ! $default_path ) {
			$default_path = OGPKWL_PATH . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR;
		}
		...
		return apply_filters( 'ogpkwl_locate_template', $template, $template_name, $template_path );
	}
} // End if().

if ( ! function_exists( 'ogpk_wishlist_template_html' ) ) {
	 ...
	 * @return string
	 */
	function ogpk_wishlist_template_html( $template_name, $args = array(), $template_path = '' ) {
		ob_start();
		ogpk_wishlist_template( $template_name, $args, $template_path );

		return ob_get_clean();
	}
}

if ( ! function_exists( 'ogpk_wishlist_get_item_data' ) ) {
	 ...
	 * @return string
	 */
	function ogpk_wishlist_get_item_data( $product, $wl_product = array(), $flat = false ) {
		$item_data      = array();
		...
		$item_data = apply_filters( 'ogpkwl_wishlist_get_item_data', $item_data, $product );
		...
		if ( 0 < count( $item_data ) ) {
			...
			if ( $flat ) {
				...
				ogpk_wishlist_template( 'ti-wishlist-item-data.php', array( 'item_data' => $item_data ) );
			}
...
if ( ! function_exists( 'ogpk_wishlist_get' ) ) {
	 ...
	 * @return array
	 */
	function ogpk_wishlist_get( $id = '', $toend = true ) {
		$wl       = new OGpkWL_Wishlist();
		$wishlist = null;
		if ( empty( $id ) ) {
			$id = get_query_var( 'ogpkwlID', null );
		}
...
if ( ! function_exists( 'ogpk_url_wishlist_default' ) ) {
	 ...
	 * @return string
	 */
	function ogpk_url_wishlist_default() {
		if ( ogpk_get_option( 'general', 'my_account_endpoint' ) ) {
			return esc_url( wc_get_endpoint_url( ogpk_get_option( 'general', 'my_account_endpoint_slug' ), '', wc_get_page_permalink( 'myaccount' ) ) );
		}

		$page = apply_filters( 'wpml_object_id', ogpk_get_option( 'page', 'wishlist' ), 'page', true ); // @codingStandardsIgnoreLine WordPress.Variables.GlobalVariables.OverrideProhibited
		if ( empty( $page ) ) {
			return '';
		}
...
if ( ! function_exists( 'ogpk_url_wishlist_by_key' ) ) {
	 ...
	 * @return string
	 */
	function ogpk_url_wishlist_by_key( $share_key, $paged = 1 ) {
		$paged = absint( $paged );
		$paged = 1 < $paged ? $paged : 1;
		$link  = ogpk_url_wishlist_default();
		if ( empty( $link ) ) {
			return $link;
		}
		...
		if ( $share_key ) {
			if ( get_option( 'permalink_structure' ) ) {
				...
				$link = add_query_arg( 'ogpkwlID', $share_key, $link );
			}
		}
...
if ( ! function_exists( 'ogpk_url_wishlist' ) ) {
	 ...
	 * @return string
	 */
	function ogpk_url_wishlist( $id = '', $paged = 1, $full = true ) {
		$share_key = $id;
		if ( ! ( is_string( $id ) && preg_match( '/^[A-Fa-f0-9]{6}$/', $id ) ) ) {
			$wishlist  = ogpk_wishlist_get( $id, false );
			$share_key = isset( $wishlist['share_key'] ) ? $wishlist['share_key'] : '';
		}

		return ogpk_url_wishlist_by_key( $share_key, $paged );
	}
}

if ( ! function_exists( 'ogpk_wishlist_status' ) ) {
	 ...
	 * @global string $page
	 */
	function ogpk_wishlist_status( $transient ) {
		if ( OGPKWL_LOAD_FREE === $transient ) {
			OGpkWL_PluginExtend::deactivate_self( OGPKWL_LOAD_FREE );

			return 'plugins.php';
		}
		if ( OGPKWL_LOAD_PREMIUM === $transient ) {
			if ( is_plugin_active( OGPKWL_LOAD_FREE ) ) {
				OGpkWL_PluginExtend::deactivate_self( OGPKWL_LOAD_FREE );
...
if ( ! function_exists( 'ogpkwl_body_classes' ) ) {
	 ...
	 * @return array
	 */
	function ogpkwl_body_classes( $classes ) {
		if ( ogpk_get_option( 'style', 'customstyle' ) ) {
			$classes[] = 'ogpkwl-theme-style';
		} else {
			$classes[] = 'ogpkwl-custom-style';
		}

		return $classes;
	}

	add_filter( 'body_class', 'ogpkwl_body_classes' );
}

if ( ! function_exists( 'ogpkwl_shortcode_addtowishlist' ) ) {
	 ...
	 * @return string
	 */
	function ogpkwl_shortcode_addtowishlist( $atts = array() ) {
		$class = OGpkWL_Public_AddToWishlist::instance();

		return $class->shortcode( $atts );
	}

	add_shortcode( 'ti_wishlists_addtowishlist', 'ogpkwl_shortcode_addtowishlist' );
}

if ( ! function_exists( 'ogpkwl_shortcode_view' ) ) {
	 ...
	 * @return string
	 */
	function ogpkwl_shortcode_view( $atts = array() ) {
		$class = OGpkWL_Public_Wishlist_View::instance();

		return $class->shortcode( $atts );
	}

	add_shortcode( 'ti_wishlistsview', 'ogpkwl_shortcode_view' );
}

if ( ! function_exists( 'ogpkwl_shortcode_products_counter' ) ) {
	 ...
	 * @return string
	 */
	function ogpkwl_shortcode_products_counter( $atts = array() ) {
		$class = OGpkWL_Public_WishlistCounter::instance();

		return $class->shortcode( $atts );
	}

	add_shortcode( 'ti_wishlist_products_counter', 'ogpkwl_shortcode_products_counter' );
}

if ( ! function_exists( 'ogpkwl_view_addto_html' ) ) {

	/**
	 * Show button Add to Wishlsit
	 */
	function ogpkwl_view_addto_html() {
		$class = OGpkWL_Public_AddToWishlist::instance();
		$class->htmloutput();
	}
}

if ( ! function_exists( 'ogpkwl_view_addto_htmlout' ) ) {

	/**
	 * Show button Add to Wishlsit, if product is not purchasable
	 */
	function ogpkwl_view_addto_htmlout() {
		$class = OGpkWL_Public_AddToWishlist::instance();
		$class->htmloutput_out();
	}
}

if ( ! function_exists( 'ogpkwl_view_addto_htmlloop' ) ) {

	/**
	 * Show button Add to Wishlsit, in loop
	 */
	function ogpkwl_view_addto_htmlloop() {
		$class = OGpkWL_Public_AddToWishlist::instance();
		$class->htmloutput_loop();
	}
}

if ( ! function_exists( 'ogpkwl_clean_url' ) ) {
	 ...
	 * @return string
	 */
	function ogpkwl_clean_url( $good_protocol_url, $original_url ) {
		return $original_url;
	}
}

if ( ! function_exists( 'ogpkwl_add_to_cart_need_redirect' ) ) {
	 ...
	 * @return boolean
	 */
	function ogpkwl_add_to_cart_need_redirect( $redirect, $_product, $redirect_url ) {
		if ( $redirect ) {
			return true;
		}
		...
		$need_url      = apply_filters( 'ogpkwl_product_add_to_cart_redirect_slug_original', remove_query_arg( 'added-to-cart', ( version_compare( WC_VERSION, '3.8.0', '<' ) ? add_query_arg( $need_url_data ) : add_query_arg( $need_url_data, '' ) ) ), $_product );
		$need_url_full = apply_filters( 'ogpkwl_product_add_to_cart_redirect_url_original', remove_query_arg( 'added-to-cart', add_query_arg( $need_url_data, $_product->get_permalink() ) ), $_product );
		...
		add_filter( 'clean_url', 'ogpkwl_clean_url', 10, 2 );
		do_action( 'before_get_redirect_url' );
		$_redirect_url = apply_filters( 'ogpkwl_product_add_to_cart_redirect_url', $_product->add_to_cart_url(), $_product );
		do_action( 'after_get_redirect_url' );
		remove_filter( 'clean_url', 'ogpkwl_clean_url', 10 );
	...
	add_filter( 'ogpkwl_product_add_to_cart_need_redirect', 'ogpkwl_add_to_cart_need_redirect', 10, 3 );
} // End if().

if ( ! function_exists( 'ogpkwl_meta_validate_cart_add' ) ) {
	 ...
	 * @return boolean
	 */
	function ogpkwl_meta_validate_cart_add( $redirect, $product, $redirect_url, $wl_product ) {
		if ( $redirect && array_key_exists( 'meta', $wl_product ) && ! empty( $wl_product['meta'] ) ) {

			$wl_product = apply_filters( 'ogpkwl_addproduct_tocart', $wl_product );

			OGpkWL_Public_Cart::prepare_post( $wl_product );
			...
			OGpkWL_Public_Cart::unprepare_post();
		}

		return $redirect;
	}

	add_filter( 'ogpkwl_product_add_to_cart_need_redirect', 'ogpkwl_meta_validate_cart_add', 90, 4 );
} // End if().

if ( ! function_exists( 'ogpk_wishlist_print_meta' ) ) {
	 ...
	 * @return string
	 */
	function ogpk_wishlist_print_meta( $meta = array(), $flat = false ) {
		if ( ! is_array( $meta ) ) {
			$meta = array();
		}
		...
		$item_data = apply_filters( 'ogpkwl_wishlist_item_meta_post', $item_data, $product_id, $variation_id );
		foreach ( $item_data as $key => $data ) {
		...
		return apply_filters( 'ogpkwl_wishlist_item_meta_wishlist', ob_get_clean() );
	}
} // End if().

if ( ! function_exists( 'ogpk_wishlistmeta' ) ) {
	 ...
	 * @return string
	 */
	function ogpk_wishlistmeta( $meta, $wl_product, $product ) {
		if ( array_key_exists( 'meta', $wl_product ) ) {
			$wlmeta = apply_filters( 'ogpkwl_wishlist_item_meta_wishlist_output', ogpk_wishlist_print_meta( $wl_product['meta'] ), $wl_product, $product );
		}
		$meta .= $wlmeta;
	...
	add_filter( 'ogpkwl_wishlist_item_meta_data', 'ogpk_wishlistmeta', 10, 3 );
}

if ( ! function_exists( 'ogpkwl_add_to_cart_item_meta_post' ) ) {
	 ...
	 * @return array
	 */
	function ogpkwl_add_to_cart_item_meta_post( $cart_item_data, $cart_item_key ) {
		$postdata = $_POST; // @codingStandardsIgnoreLine WordPress.VIP.SuperGlobalInputUsage.AccessDetected

		$postdata = apply_filters( 'ogpkwl_product_prepare_meta', $postdata );
		...
		$cart_item_data['ogpkwl_formdata'] = $postdata;

		return $cart_item_data;
	}

	add_action( 'woocommerce_add_cart_item', 'ogpkwl_add_to_cart_item_meta_post', 10, 2 );
} // End if().

if ( ! function_exists( 'ogpkwl_set_utm' ) ) {

	/**
	 * Set UTM sources.
	 */
	function ogpkwl_set_utm() {

		// Forcing partners UTM.
		if ( class_exists( 'Ocean_Extra' ) && ! defined( 'OGPKWL_PARTNER' ) && ! defined( 'OGPKWL_CAMPAIGN' ) ) {
			define( 'OGPKWL_PARTNER', 'oceanwporg' );
			define( 'OGPKWL_CAMPAIGN', 'oceanwp_theme' );
		}

		// Set a source.
		$source = get_option( OGPKWL_PREFIX . '_utm_source' );
		if ( ! $source || $source !== defined( 'OGPKWL_PARTNER' ) ) {
			$source = defined( 'OGPKWL_PARTNER' ) ? OGPKWL_PARTNER : 'wordpress_org';
			update_option( OGPKWL_PREFIX . '_utm_source', $source );
		}

		define( 'OGPKWL_UTM_SOURCE', $source );

		// Set a medium.
		$medium = get_option( OGPKWL_PREFIX . '_utm_medium' );
		if ( ! $medium || ( 'organic' === $medium && defined( 'OGPKWL_PARTNER' ) ) ) {
			$medium = defined( 'OGPKWL_PARTNER' ) ? 'integration' : 'organic';
			update_option( OGPKWL_PREFIX . '_utm_medium', $medium );
		}

		define( 'OGPKWL_UTM_MEDIUM', $medium );

		// Set a campaign.
		$campaign = get_option( OGPKWL_PREFIX . '_utm_campaign' );
		if ( ! $campaign || $campaign !== defined( 'OGPKWL_CAMPAIGN' ) ) {
			$campaign = defined( 'OGPKWL_PARTNER' ) ? ( defined( 'OGPKWL_CAMPAIGN' ) ? OGPKWL_CAMPAIGN : OGPKWL_PARTNER ) : 'organic';
			update_option( OGPKWL_PREFIX . '_utm_campaign', $campaign );
		}

		define( 'OGPKWL_UTM_CAMPAIGN', $campaign );
	}
} // End if().
...
if ( ! function_exists( 'is_wishlist' ) ) {
	...
	function is_wishlist() {
		return ( is_page( apply_filters( 'wpml_object_id', ogpk_get_option( 'page', 'wishlist' ), 'page', true ) ) );
	}
}

if ( ! function_exists( 'ogpkwl_get_wishlist_products' ) ) {
	/**
	 ...
	 * @param array $data query parameters for get() method of OGpkWL_Product() class.
	 *
	 * @return array|bool
	 */
	function ogpkwl_get_wishlist_products( $wishlist_id = 0, $data = array() ) {
		$wishlist = ogpk_wishlist_get( $wishlist_id );
		if ( empty( $wishlist ) ) {
			return false;
		}
		$wlp      = new OGpkWL_Product( $wishlist );
